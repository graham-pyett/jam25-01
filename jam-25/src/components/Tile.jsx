...
const Tile = ({ sx, letter, id }) => {
  ...
  return (
    <Box
      ...
      {...attributes}
    >
      <Tooltip arrow open={!!scoringTiles?.find((t) => t.id === id)} title={<span style={{ fontFamily: 'Orbitron', fontSize: 16, color: scoringTiles?.find((t) => t.id === id)?.score < 0 ? '#ff9ca7' : '#b3faaa'}}>{scoringTiles?.find((t) => t.id === id)?.score >= 0 ? '+' : ''} {scoringTiles?.find((t) => t.id === id)?.score ?? ''}</span>} placement={scoringTiles?.find((t) => t.id === id)?.placement}>
        {
          (letter.multiplier) && (
